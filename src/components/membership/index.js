...
const stripePromise = loadStripe("pk_test_51QErOaDfE2BaMXMyntat71EdVfgYxlFwTqf02AFkF7K7XYyfIV6vesqCkoYsEJi2dl6rqtVhQHDOY75XNqjBaOpW00uBJRKwFK")
...
function Membership({profileInfo}) {
  ...
  async function updateProfile() {
    ...
    await updateProfileAction({
      ...profileInfo, isPremiumUser  : true,
      memberShipType : fetchCurrentPlanFromSessionStorage?.type,
      memberShipStartDate : new Date().toString(),
      memberShipEndDate : new Date(
        new Date().getFullYear() + fetchCurrentPlanFromSessionStorage?.type === 'basic' ? 1 : fetchCurrentPlanFromSessionStorage?.plan === 'teams' ? 2 : 5, 
        new Date().getMonth(),
        new Date().getDate()
      )
    },'/membership')
          ...
          <div className="grid grid-cols-1 gap-x-4 gap-y-8 md:grid-cols-2 lg:grid-cols-3" >
            {
              ...
              footerContent={ 
                ...
                profileInfo?.memberShipType === 'enterprise' || (
                  ...
                  profileInfo?.memberShipType === 'teams' && index >= 0 && index <2 ? null : <Button onClick={ () => handlePayment(plan) } className='disabled:opacity-65 flex h-11 items-center justify-center' > 
