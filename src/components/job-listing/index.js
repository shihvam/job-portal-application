                      ...
                      <MenubarContent>
                        {
                          filterMenu.options.map( (option, optionIndex) => (
                            ...
                            >
                              <div className={`h-4 w-4 border rounded border-gray-900 text-indigo-600 ${filterParams && Object.keys(filterParams).length > 0 && filterParams[filterMenu.id] && filterParams[filterMenu.id].indexOf(option) > -1 ? "bg-black" : ''  } `}/ > 

                              <Label className=" ml-3 cursor-pointer text-sm ext-gray-600 " > {option} </Label>
                            </MenubarItem>
                          ))
                        }
                      </MenubarContent>
                  ...
                  <div className="grid grid-cols-1 gap-x-4 gap-y-8 md:grid-cols-2 lg:grid-cols-3" > 
                    {
                      ...
                      jobList.map( jobItem => profileInfo?.role === 'candidate' ? 
                         <CandidateJobCard  jobItem={jobItem}  jobApplications={jobApplications} profileInfo = {profileInfo}  />  : 
                       <RecruiterJobCard profileInfo={profileInfo} jobItem={jobItem} jobApplications={jobApplications} />  
                    ) 
                      : null
                    }
                  </div>
